...
import { allIntegrations } from './integrations/platforms';

const prisma = new PrismaClient();

const PASSWORD_SALT_ROUNDS = 12;
const SEED_MOCK_DATA = process.env['SEED_MOCK_DATA'] === 'true'; // Default to false for production
...
async function main() {
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    console.log('Creating real platform integrations...');

    let sampleIntegration: { id: string; displayName: string } | null = null;
    ...
    for (const integration of allIntegrations) {
        const created = await prisma.integration.upsert({
            ...
            update: {
                ...
                sourceConfig: integration.sourceConfig as any,
                listPageConfig: integration.listPageConfig as any,
                fieldMappings: integration.fieldMappings as any,
        ...
        console.log(`  ✓ Created integration: ${created.displayName}`);

        // Keep reference to first integration for mock listings
        if (!sampleIntegration) {
            sampleIntegration = created;
        }
    }

    // Also keep the sample platform for mock data
    const mockIntegration = await prisma.integration.upsert({
        where: { name: 'sample-platform' },
        update: {},
        create: {
            name: 'sample-platform',
            displayName: 'Sample Platform (Mock Data)',
            enabled: SEED_MOCK_DATA,
            sourceConfig: {
    ...
    console.log(`  ✓ Created mock integration: ${mockIntegration.displayName}`);
    ...
    if (SEED_MOCK_DATA) {
        ...
        console.log('  ⏭️ Skipping mock listings (SEED_MOCK_DATA=false)');
    }
